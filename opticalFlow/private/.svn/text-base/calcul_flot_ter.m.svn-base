function [flows]=calcul_flot_ter(validloc,faces,vertices,intv,F,Time,dim,graphe,temps,lambda,Compindex,MOVIE)
...
% - Compindex : computes the Poincaré index on each triangle
...
[cont_grad_v,grad_v,tangent_basis,tg_basis,aires,norm_faces,norm_vertices,index1,index2]=matrices_contraintes_ter(faces,vertices,dim);
if length(lambda)<=1
